...
<script>
    export default {
        ...
        methods: {
            getData:function(){
                this.$http.get('/admin/feedback/get/'+this.id).then(response => {
            ...
            pageSave: function () {
                if (this.$validator.run(this.list, this.rules)) {
                    ...
                    this.$http.post('/admin/feedback/save/'+this.id, this.list).then(response => {
                        let item  = response.data;
                        if(item.id) {
                            UIkit.notification({message: 'Отзыв успешно сохранен!', status: 'success'});
                        }
                    });
                }
            },
