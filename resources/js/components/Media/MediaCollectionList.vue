<template>
	<div>
		<header class="uk-content-header uk-background-default">
			<div class="title"><h2>Коллекции медиа файлов
				<div class="uk-inline">
					<button class="uk-button uk-button-primary uk-button-small uk-border-rounded" type="button" uk-icon="plus"></button>
					<div uk-dropdown>
						<ul class="uk-nav uk-nav-default" >
							<li class="">
								<a href="/admin/media_collection/add" title="Страницу">
									<i uk-icon="template"></i><span>Добавить коллекцию</span>
								</a>
							</li>
						</ul>
					</div>
		...
		<div class="uk-margin-top">
			...
			<ul class="uk-comment-list">
				<li v-for="item in list">
					<media-collection-list-item :item="item" :key="item.id" @Delete="Delete"></media-collection-list-item>
				</li>
			</ul>
...
<script>
    import MediaCollectionListItem from "./MediaCollectionListItem";
    export default {
        components: {MediaCollectionListItem},
        props: ['current_page'],
        data() {
            return {
                list: [],
        ...
        methods: {
            getList:function () {
                this.$http.get('/admin/media_collection/get/' + this.current_page).then(response => {
